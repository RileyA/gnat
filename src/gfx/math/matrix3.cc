...
void Matrix3::FromAxes(const Vector3& x_axis,
...
Vector3 Matrix3::operator*(const Vector3& point) const {
  ...
  for (size_t row = 0; row < 3; row++) {
    prod[row] =
        m[row][0] * point[0] + m[row][1] * point[1] + m[row][2] * point[2];
  }
  return prod;
}
//-----------------------------------------------------------------------
...
Vector3 operator*(const Vector3& point, const Matrix3& matrix) {
  Vector3 prod(0, 0, 0);
  for (size_t row = 0; row < 3; row++) {
    prod[row] = point[0] * matrix.m[0][row] + point[1] * matrix.m[1][row] +
...
bool Matrix3::Inverse(Matrix3& inverse, Real tolerance) const {
  ...
  Real det = m[0][0] * inverse[0][0] + m[0][1] * inverse[1][0] +
             m[0][2] * inverse[2][0];
