...
public:
  ...
  scoped_ptr() : ptr_(0), ref_count_(0) {}
  //---------------------------------------------------------------------------

  explicit scoped_ptr(T *p) : ptr_(0), ref_count_(0) { bind(p); }
  //---------------------------------------------------------------------------
  ...
  scoped_ptr(const scoped_ptr<T> &rhs) : ptr_(0), ref_count_(0) {
    ptr_ = rhs.get();
  ...
  /** copy constructor (templated so polymorphism works) */
  template <typename Y>
  scoped_ptr(const scoped_ptr<Y> &rhs)
      : ptr_(0), ref_count_(0) {
    ptr_ = rhs.get();
    ref_count_ = rhs.GetRefCountPtr();
    if (ref_count_)
  ...
  inline unsigned int *GetRefCountPtr() const { return ref_count_; }
  //---------------------------------------------------------------------------

  inline unsigned int GetRefCount() const { return *ref_count_; }
  //---------------------------------------------------------------------------

  inline bool IsNull() { return !ptr_; }
  //---------------------------------------------------------------------------
